...
function parse<T>(input: string, ini: (ch: string) => T): T[][] {
    return input.trim().split("\n").map(line => Array.from(line.trim()).map(ini));
}
...
export function solveA(input: string): number {
    const board = parse(input, field => [field, null] as [field: string, component: number | null]);
    ...
    return componentStats.map(([area, internalFencesCountedTwice]) => area * (area * 4 - internalFencesCountedTwice)).reduce((a, b) => a + b);
}
...
export function solveB(input: string): number {
    ...
    return componentStats.map(([area, sides]) => area * sides).reduce((a, b) => a + b);
}
