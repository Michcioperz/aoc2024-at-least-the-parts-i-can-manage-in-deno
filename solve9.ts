...
export function solveA(input: string): number {
  const diskmap = Array.from(input.trim()).flatMap((x, i) =>
    Array(parseInt(x)).fill((i & 1) ? null : i / 2)
  );
  let checksum = 0;
...
export function solveB(input: string): number {
  ...
  for (let fileId = fileCount - 1; fileId > 0; fileId--) {
    ...
    for (const [otherStart, otherLen] of files) {
      ...
      if (
        files.every(([otherStart, otherLen]) =>
          !rangesCollide(newStart, len, otherStart, otherLen)
        )
      ) {
        minStart = newStart;
      }
...
function rangesCollide(
  lhsStart: number,
  lhsLen: number,
  rhsStart: number,
  rhsLen: number,
): boolean {
  return Math.min(lhsStart + lhsLen, rhsStart + rhsLen) >
    Math.max(lhsStart, rhsStart);
}
